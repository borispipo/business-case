...
const requestHandler = {handleError};
...
requestHandler.put = (Model)=>{
    return async (req,res)=>{
        try {
            ...
            const data = await Model.runValidators({[Model.primaryKey]:primaryKeyValue},toUpdate,{runValidators:true}).exec();
            return res.json(data);
        } catch(e){
            return handleError(e,res);
        }
